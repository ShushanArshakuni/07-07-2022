...
function calcSumOfSalaries(employees) {
  let sumOfSalaries = employees.reduce(
    (accum, item) => accum + +item.salary.slice(1),
    0
  );
  return "$" + Math.round(sumOfSalaries);
}
console.log(calcSumOfSalaries(employees));
